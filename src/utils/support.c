 ...
 */
inline void * newPrimObject(int dataSize) {
  ObjRef bigObjRef = heap_alloc(dataSize);
  if (bigObjRef == NULL)
    fatalError("newPrimObject() got no memory");
  return bigObjRef;
}

inline void * getPrimObjectDataPointer(void * obj){
  return ((ObjRef)obj)->data;
}

inline int getPrimSize(void* obj) {
  return ((ObjRef)obj)->size;
}